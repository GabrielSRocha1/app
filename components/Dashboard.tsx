...
const Dashboard: React.FC<DashboardProps> = ({ transactions, currentMonth, onMonthChange }) => {
  ...
  // Gerar os últimos 6 meses dinamicamente para o gráfico de barras
  ...
  // Seletor de meses no estilo "Janela Tripla" (Igual ao Reports/Flows)
  ...
  return (
    <div className="space-y-6 pb-6 animate-in fade-in duration-700">
      ...
      {/* Seletor de Mês (Estilo Reports) */}
      <div className="flex items-center justify-center gap-4 px-4 py-2">
         <button 
           onClick={handlePrevMonth}
           disabled={currentMonth <= minDate}
           className="w-12 h-12 flex-shrink-0 flex items-center justify-center bg-[#111] border border-white/10 rounded-xl text-gray-400 active:scale-95 transition-all disabled:opacity-10"
         >
           <ChevronLeft size={20} />
         </button>
            ...
            {visibleMonths.map((m, i) => {
              ...
              return (
                <button 
                  ...
                  className={`flex-shrink-0 px-4 py-3 rounded-xl text-[10px] font-black uppercase transition-all whitespace-nowrap ${isActive ? 'bg-[#1a1a1a] border border-white/10 text-white shadow-lg' : 'text-gray-700 hover:text-gray-500'}`}
                >
                  {m.label}
                </button>
              );
      ...
      <div className="bg-[#111] mx-4 p-6 rounded-[28px] border border-white/5 shadow-2xl">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2 bg-[#1a1a1a] px-3 py-1 rounded-lg text-[10px] font-black text-gray-400 uppercase tracking-tighter">
            Tendência Semestral
          </div>
        </div>
          ...
          {chartData.map((bar, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-3 h-full">
              <div className="w-full flex flex-col justify-end gap-1 h-full">
                <div 
                  className={`w-full rounded-t-lg transition-all duration-1000 ease-out ${bar.active ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.3)]' : 'bg-green-500/10'}`} 
                  style={{ height: bar.hasData ? `${(bar.income / maxVal) * 100}%` : '4%' }}
                ></div>
                <div 
                  className={`w-full rounded-b-lg transition-all duration-1000 ease-out ${bar.active ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'bg-red-500/10'}`} 
                  style={{ height: bar.hasData ? `${(bar.expense / maxVal) * 60}%` : '4%' }}
                ></div>
              </div>
              <span className={`text-[10px] font-black uppercase tracking-tighter transition-colors ${bar.active ? 'text-white' : 'text-gray-700'}`}>
                {bar.label}
              </span>
            </div>
          ))}
